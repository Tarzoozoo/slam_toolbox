...
/*****************************************************************************/
{
  ...
  if (name != "") {
    ...
    int rc = system(("ros2 run nav2_map_server map_saver_cli -f " + name  + " --ros-args -p map_subscribe_transient_local:=true -p save_map_timeout:=10000").c_str());
    if (rc == 0) {
    ...
    int rc = system("ros2 run nav2_map_server map_saver_cli --ros-args -p map_subscribe_transient_local:=true -p save_map_timeout:=10000");
    if (rc == 0) {
      response->result = response->RESULT_SUCCESS;
    } else {
      response->result = response->RESULT_UNDEFINED_FAILURE;
